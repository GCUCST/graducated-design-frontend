      ...
      >
        ...
        <el-table-column prop="major" 
         :filters="majorFileter"
          :filter-method="filterHandler"
        sortable label="专业"></el-table-column>
        <el-table-column prop="adminClass"
          ...
          :filter-method="filterHandler"
         sortable label="班级" show-overflow-tooltip></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getGSMA() {
      ...
      axios
        ...
        .then(function(response) {
          ...
          that.adminClassJson = JSON.parse(response.data.object.gsmajson).adminClass;
          
          var majorJson =  JSON.parse(response.data.object.gsmajson).major
          for(var i = 0;i<majorJson.length;i++){
            that.majorFileter.push({text:majorJson[i].name,value:majorJson[i].name})
          }
          var gradeJson =  JSON.parse(response.data.object.gsmajson).grade
          for(var i = 0;i<gradeJson.length;i++){
            that.gradeFilter.push({text:gradeJson[i].name,value:gradeJson[i].name})
          }
    ...
    save() {
      ...
      if(this.oneStudent.adminClass==null||this.oneStudent.adminClass==''){
        alert("请输入学生班级！")
        return ;
      }
      ...
      axios
        ...
        that.temp_item=null,  
         that.oneStudent={ stuId: "", name: "",gender:1, major:"",grade:"",adminClass:""} 
